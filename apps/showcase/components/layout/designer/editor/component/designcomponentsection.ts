...
@Component({
    selector: 'app-design-component-section',
    standalone: true,
        ...
        <div class="grid grid-cols-4 gap-x-2 gap-y-3">
            @for (entry of objectKeys(tokens()); track entry) {
                @if (!isObject(tokens()[entry])) {
                    <app-design-token-field [(modelValue)]="tokens()[entry]" [label]="camelCaseToSpaces(entry)" [componentKey]="componentKey()" [path]="path() + '.' + entry" [type]="isColor(entry) ? 'color' : null" [switchable]="true" />
                }
            }
        </div>
        @if (hasNestedTokens()) {
            @for (entry of objectKeys(nestedTokens()); track entry) {
                <app-design-component-section [componentKey]="componentKey()" [path]="path() + '.' + entry" class="block mt-3" />
            }
        }
    </section>`
})
