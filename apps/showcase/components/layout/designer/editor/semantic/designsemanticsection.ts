...
@Component({
    selector: 'app-design-semantic-section',
    standalone: true,
        ...
        <section>
            ...
            @if (hasPrimitiveTokens()) {
                <div class="grid grid-cols-4 gap-x-2 gap-y-3">
                    @for (item of primitiveTokensArray(); track item) {
                        <app-design-token-field [(modelValue)]="tokens()[item.key]" [label]="camelCaseToSpaces(item.key)" [path]="path + '.' + item.key" [switchable]="switchable" />
                    }
                </div>
            }
            @if (hasNestedTokens()) {
                @for (item of nestedTokensArray(); track item) {
                    <app-design-semantic-section [path]="path + '.' + item.key" [switchable]="switchable" class="block mt-3" />
                }
            }
        </section>
    `,
