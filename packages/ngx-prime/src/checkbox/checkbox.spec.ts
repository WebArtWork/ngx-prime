...
class TestNativeCheckboxDirectiveComponent {
    checked = false;
    inputId = 'native-checkbox';
    indeterminate = false;
    readonly = false;
    changeEvent: CheckboxChangeEvent | undefined;
    focusEvent: Event | undefined;
...
class TestSignalFormCheckboxDirectiveComponent {
    model = signal({ value: false });
    checkboxForm = form(this.model);
}

@Component({
    imports: [CheckboxContainerDirective, CheckboxDirective, CheckboxIconDirective, FormsModule],
    template: `
        <label pCheckboxContainer>
            <input type="checkbox" pCheckbox [(ngModel)]="checked" [binary]="true" />
            <span pCheckboxIcon><i class="pi pi-star"></i></span>
        </label>
    `
})
class TestNativeCheckboxIconComponent {
    checked = false;
}
...
describe('Checkbox', () => {
    ...
    describe('CSS Classes and Styling Tests', () => {
        ...
        it('should apply custom styles', async () => {
            ...
            const inputStyleValue = checkboxInstance.inputStyle();

            if (inputStyleValue) {
                Object.keys(inputStyleValue).forEach((key) => {
                    inputElement.nativeElement.style[key] = inputStyleValue[key];
                });
            }
...
describe('CheckboxDirective', () => {
    let fixture: ComponentFixture<TestNativeCheckboxDirectiveComponent>;
    let component: TestNativeCheckboxDirectiveComponent;
    let input: HTMLInputElement;

    beforeEach(async () => {
        await TestBed.configureTestingModule({
            imports: [TestNativeCheckboxDirectiveComponent],
            providers: [provideZonelessChangeDetection()]
        }).compileComponents();

        fixture = TestBed.createComponent(TestNativeCheckboxDirectiveComponent);
        component = fixture.componentInstance;
        fixture.detectChanges();
        input = fixture.nativeElement.querySelector('input');
    });

    it('maps compatibility state to the native checkbox and emits native events', () => {
        expect(input.id).toBe('native-checkbox');
    ...
    it('preserves its value when readonly and clears indeterminate after a user change', () => {
...
describe('CheckboxDirective Signal Forms', () => {
    it('updates a bound signal-form field from native checkbox interaction', async () => {
        await TestBed.configureTestingModule({
            imports: [TestSignalFormCheckboxDirectiveComponent],
            providers: [provideZonelessChangeDetection()]
        }).compileComponents();
        ...
        expect(fixture.componentInstance.model().value).toBe(true);
    });
});

describe('Checkbox native icon directives', () => {
    it('keeps the input native while supporting a themed custom icon container', async () => {
        await TestBed.configureTestingModule({
            imports: [TestNativeCheckboxIconComponent],
            providers: [provideZonelessChangeDetection()]
        }).compileComponents();
        ...
        const icon = fixture.nativeElement.querySelector('[pCheckboxIcon]') as HTMLSpanElement;

        expect(label.classList.contains('p-checkbox')).toBe(true);
        expect(input.classList.contains('p-checkbox-input')).toBe(true);
        expect(icon.classList.contains('p-checkbox-box')).toBe(true);

        input.click();
        await fixture.whenStable();

        expect(fixture.componentInstance.checked).toBe(true);
        expect(label.classList.contains('p-checkbox-checked')).toBe(true);
    });
});
