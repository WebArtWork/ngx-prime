...
describe('MeterGroup', () => {
    ...
    describe('Value and Meter Rendering', () => {
        ...
        it('should apply meter styles correctly for vertical orientation', () => {
            (meterGroup as any).orientation = () => 'vertical';
    ...
    describe('Edge Cases', () => {
        ...
        it('should handle undefined value', () => {
            (meterGroup as any).value = () => undefined;
            expect(meterGroup.totalPercent()).toBe(0);
        });

        it('should handle null value gracefully', () => {
            (meterGroup as any).value = () => null;
        ...
        it('should handle boundary values', () => {
            // Test with min = max
            (meterGroup as any).min = () => 50;
            (meterGroup as any).max = () => 50;
            const percent = meterGroup.percent(50);

            expect(percent).toBe(100); // When min = max, any value should be 100%

            // Reset to normal
            (meterGroup as any).min = () => 0;
            (meterGroup as any).max = () => 100;
