...
@Component({
    template: `
        <input #uploader="pFileUpload" type="file" pFileUpload [(value)]="files" multiple maxFileSize="2" (onError)="errors.push($event.reason)" />
        <button pFileUploadClear [pFileUploadClear]="uploader">Clear</button>
    `,
    imports: [FileUploadDirective, FileUploadClearDirective]
})
class NativeFileUploadHost {
    files: File[] = [];
    errors: string[] = [];
}

describe('Native FileUploadDirective', () => {
    let fixture: ComponentFixture<NativeFileUploadHost>;

    beforeEach(async () => {
        await TestBed.configureTestingModule({
            imports: [NativeFileUploadHost],
            providers: [provideZonelessChangeDetection()]
        }).compileComponents();

        fixture = TestBed.createComponent(NativeFileUploadHost);
        fixture.detectChanges();
    });

    it('stores the native selection and emits selected files', () => {
        const input = fixture.nativeElement.querySelector('input') as HTMLInputElement;
        const selected = new File(['a'], 'a.txt', { type: 'text/plain' });

        Object.defineProperty(input, 'files', { configurable: true, value: [selected] });

        input.dispatchEvent(new Event('change'));

        expect(fixture.componentInstance.files).toEqual([selected]);
        expect(fixture.debugElement.query(By.directive(FileUploadDirective)).componentInstance.value()).toEqual([selected]);
    });

    it('rejects invalid selections and clears the native input', () => {
        const input = fixture.nativeElement.querySelector('input') as HTMLInputElement;
        const oversized = new File(['abc'], 'oversized.txt', { type: 'text/plain' });
        ...
        expect(fixture.componentInstance.errors).toEqual(['maxFileSize']);
        expect(input.value).toBe('');
    });

    it('clears the selection with pFileUploadClear', () => {
        const input = fixture.nativeElement.querySelector('input') as HTMLInputElement;
        const selected = new File(['a'], 'a.txt', { type: 'text/plain' });
        ...
        expect(fixture.componentInstance.files).toEqual([]);
        expect(fixture.debugElement.query(By.directive(FileUploadDirective)).componentInstance.value()).toEqual([]);
    });
});
