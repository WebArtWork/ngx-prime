...
class TestNativeRadioButtonDirectiveComponent {
    value = '';
    radioValue = 'native-value';
    inputId = 'native-radio';
    binary = false;
    disabled = false;
    required = true;
    invalid = true;
...
class TestSignalFormRadioButtonDirectiveComponent {
    model = signal({ value: 'one' });
    radioForm = form(this.model);
}
...
describe('RadioButtonDirective', () => {
    let fixture: ComponentFixture<TestNativeRadioButtonDirectiveComponent>;
    let component: TestNativeRadioButtonDirectiveComponent;
    let input: HTMLInputElement;

    beforeEach(async () => {
        await TestBed.configureTestingModule({
            imports: [TestNativeRadioButtonDirectiveComponent],
            providers: [provideZonelessChangeDetection()]
        }).compileComponents();

        fixture = TestBed.createComponent(TestNativeRadioButtonDirectiveComponent);
        component = fixture.componentInstance;
        fixture.detectChanges();
        input = fixture.nativeElement.querySelector('input');
    });

    it('keeps native radio Forms behavior and emits interaction events', () => {
        expect(input.id).toBe('native-radio');
        ...
        expect(input.classList.contains('p-radiobutton-sm')).toBe(true);

        input.click();
        input.dispatchEvent(new Event('focus'));
        input.dispatchEvent(new Event('blur'));

        expect(component.value).toBe('native-value');
        expect(component.clickEvent?.value).toBe('native-value');
        expect(component.focusEvent).toBeDefined();
        expect(component.blurEvent).toBeDefined();
    });

    it('does not emit or update a disabled native radio', () => {
        ...
        expect(component.clickEvent).toBeUndefined();
        expect(input.disabled).toBe(true);
    });

    it('supports the binary compatibility value', () => {
...
describe('RadioButtonDirective Signal Forms', () => {
    it('updates a bound signal-form field from native radio interaction', async () => {
        await TestBed.configureTestingModule({
            imports: [TestSignalFormRadioButtonDirectiveComponent],
            providers: [provideZonelessChangeDetection()]
        }).compileComponents();

        const fixture = TestBed.createComponent(TestSignalFormRadioButtonDirectiveComponent);
        ...
        expect(fixture.componentInstance.model().value).toBe('two');
    });
});
